...
class bpeace():
    ...
    def __init__(self, study):
        ...
        if study == 'bpeace1':
            ...
            self.beacon_list = [30,1,21,34,22,28,24,41,26,48,46,25,15,44,23,29,10,16,36,38,40,5,17,6,19,32,11,7]
    ...
    def process_accelerometer(self,data_dir='/Volumes/HEF_Dissertation_Research/utx000/bpeace1/beiwe/accelerometer/',resample_rate=100):
        ...
        for pt in os.listdir(data_dir)[:3]:
            if len(pt) == 8:
                pt_df = pd.DataFrame()
...
def main():
    ...
    print('\t20. BPEACE2 REDCap Environment and Experiences Survey')

    ans = int(input('Answer: '))
    ut1000_processor = ut1000()
    ut2000_processor = ut2000()
    ut3000_processor = ut3000()
    bpeace1_processor = bpeace('bpeace1')
    bpeace2_processor = bpeace('bpeace2')
    ...
    # BPEACE1 Beacon Data
    if ans == 4 or ans == 5:
        if bpeace1_processor.process_beacon(beacon_list=bpeace1_processor.beacon_list):
    ...
    if ans == 4 or ans == 10:
        if bpeace1_processor.process_power_state():
    ...
    # BPEACE1 Wifi Data
    if ans == 4 or ans == 11:
        if bpeace1_processor.process_wifi():
    ...
    if ans == 4 or ans == 12:
        if bpeace1_processor.process_reachability():
    ...
    # BPEACE2 Beacon Data
    if ans == 13 or ans == 14:
        if bpeace2_processor.process_beacon(data_dir='../../data/raw/bpeace2/beacon/',beacon_list=bpeace2_processor.beacon_list):
    ...
    # BPEACE2 survey Data
    if ans == 13 or ans == 15:
        if bpeace2_processor.process_weekly_surveys(data_dir='../../data/raw/bpeace2/beiwe/survey_answers/'):
    ...
    # BPEACE2 gps Data
    if ans == 13 or ans == 17:
        if bpeace2_processor.process_gps(data_dir='/Volumes/HEF_Dissertation_Research/utx000/bpeace2/beiwe/gps/'):
    ...
    if ans == 4 or ans == 18:
        if bpeace1_processor.process_accelerometer(data_dir='/Volumes/HEF_Dissertation_Research/utx000/bpeace2/beiwe/accelerometer/'):
            logger.info(f'Data for BPEACE1 accelerometer processed')
        else:
            logger.error(f'Data for BPEACE1 accelerometer NOT processed')
